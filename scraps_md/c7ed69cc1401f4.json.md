...
このまとめを書き始めたきっかけは、[OpenFrameworks](https://openframeworks.cc/)をVulkan/DirectX12/Metalなどの低レベル描画APIに対応させるには、実はOpenGLごと低レベル描画APIにポートするのが一番なのではないかと思ったこと。\n\nそうすれば、大半はOpenGLで書いておきながらも、必要なところで低レベル描画APIが呼べるので、ボトルネックを回避できる。そして、徐々に完全移行したり、フォールバックとしてOpenGLを書くという道筋ができる。…と考えた
...
OpenGL on Metal\nhttps://github.com/openglonmetal/MG
...
OpenGL on Vulkan\nhttps://docs.mesa3d.org/drivers/zink.htm
...
一応別のアプローチとして、GLとVulkanを共存させるノウハウがまとめられたリポジトリ。\nhttps://github.com/nvpro-samples/gl_vk_simple_intero
